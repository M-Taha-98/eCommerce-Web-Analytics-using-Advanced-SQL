...
with growth_summary as (
select year(ws.created_at) as yr, quarter(ws.created_at) as qtr,
		round((count(distinct o.order_id)/count(distinct ws.website_session_id))*100, 2) as session_to_order_cnv_rt,
...
select *,  
round((session_to_order_cnv_rt - lag(session_to_order_cnv_rt) over(order by yr asc, qtr asc))/lag(session_to_order_cnv_rt) over(order by yr asc, qtr asc)*100, 1) as QoQ_cnv_rt_growth
from growth_summary;

	-- session_to_order_cnv rate has seen more than 1.75x growth since the launch of business, Q1 of 2023 saw highest QoQ growth of nearly 29%.
    -- average order value has increased steadily as product portfolio has grown over time, the most significant growth seen in Q1 of 2024 of approx $7.4.
		-- which also increased the revenue/session to $4.08 from $3.53 in the previous quarter.
...
from growth_summary;
  ...
  -- Orders from brand searches, organic traffic and direct type-in traffic has been growing steadily to-date which is indicative of brand-recognition and 
	-- that the business has been able to establish value amongst customers.
...
-- focus on Gsearch visitors since they form bulk of traffic.
-- launch similar products that have performed well.
